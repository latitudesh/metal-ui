...
const AlgoliaSearch = (props) => {
  const { ALGOLIA_APP_ID, ALGOLIA_API_SEARCH_KEY } = props;

  if (!ALGOLIA_APP_ID || !ALGOLIA_API_SEARCH_KEY) {
    return (
      <>
        {!ALGOLIA_APP_ID && (
          <h1>ALGOLIA_APP_ID prop was not found and is required.</h1>
        )}
        {!ALGOLIA_API_SEARCH_KEY && (
          <h1>ALGOLIA_API_SEARCH_KEY prop was not found and is required.</h1>
        )}
      </>
    );
  }
